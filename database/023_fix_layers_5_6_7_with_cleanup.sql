-- ============================================================================
-- Migration: 023_fix_layers_5_6_7_with_cleanup.sql
...
--
-- SAFETY NOTES:
-- - Deletes only user test data (user_quiz_progress, user_quiz_results)
-- - Keeps ALL database structure intact (tables, columns, constraints)
-- - Keeps ALL backend logic intact (API, filtering, scoring)
-- - Keeps ALL iOS app functionality intact
-- ============================================================================
...
-- Delete all user quiz progress (in-progress quiz data)
DELETE FROM user_quiz_progress;

-- Delete all user quiz results (completed quiz data)
DELETE FROM user_quiz_results;

-- Verification: Show that user data is cleared
...
-- ============================================================================
-- STEP 2: DELETE ALL INCORRECT QUESTIONS IN LAYERS 5, 6, 7
-- ============================================================================

-- Delete Layer 5 incorrect questions (Q26-Q31) and their options
...
-- Delete Layer 6 incorrect questions (Q32-Q38) and their options
DELETE FROM quiz_options WHERE question_id IN (
    SELECT id FROM quiz_questions WHERE layer_number = 6
);
DELETE FROM quiz_questions WHERE layer_number = 6;

-- Delete Layer 7 incorrect questions (Q39-Q45) and their options
DELETE FROM quiz_options WHERE question_id IN (
...
-- ============================================================================
-- STEP 3: INSERT CORRECT LAYER 5 QUESTIONS (NEURODIVERGENCE ASSESSMENT)
...
-- ============================================================================
-- STEP 4: INSERT CORRECT LAYER 6 QUESTIONS (MINDSET & PERSONALITY)
...
-- ============================================================================
-- STEP 5: INSERT CORRECT LAYER 7 QUESTIONS (VALUES & BELIEFS)
...
-- Verify all tables still exist (structure intact)
SELECT 'Database Structure Intact' as verification,
       (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'quiz_questions') as quiz_questions_exists,
       (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'quiz_options') as quiz_options_exists,
       (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'user_quiz_progress') as user_quiz_progress_exists,
       (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'user_quiz_results') as user_quiz_results_exists;
